...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, emp => emp.Mgr, emp => emp.EmpNo,
            (emp, emp1) => new { Employee = emp.EName, Manager = emp1.EName });
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps.Select(emp => new { emp.EName, Total = emp.Comm + emp.Sal }).ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            ...
            .SelectMany(join =>
                grades
                    .Where(salgrade => salgrade.Losal <= join.emp.Sal && join.emp.Sal <= salgrade.Hisal)
                    .Select((salgrade) => new { join.emp.EName, salgrade.Grade, join.dept.DName }))
            .ToList();
